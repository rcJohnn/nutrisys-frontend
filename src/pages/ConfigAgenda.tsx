...
<<<<<<< HEAD
  const handleGuardarTodoHorario = async () => {
    ...
    try {
      await Promise.all(horarioSemanal.map(dia => guardarHorarioSemanal({
  ...
  const handleGuardarTodosTiempos = async () => {
    if (!medicoId || tiemposComida.length === 0) return;
    setIsSavingTiempos(true);
    try {
      // Secuencial para evitar deadlocks en el MERGE del SP
      for (const tc of tiemposComida) {
        await guardarTiempoComida({
          Id_Medico: medicoId,
          Tipo_Comida: tc.Tipo_Comida,
          Hora_Inicio: tc.Hora_Inicio,
          Hora_Fin: tc.Hora_Fin,
          Activo: tc.Activo,
          IdUsuarioGlobal: userId,
        });
      }
      queryClient.invalidateQueries({ queryKey: ['tiempos-comida', medicoId] });
      alert('Tiempos de comida guardados correctamente');
    } catch (err: any) {
      // normalizeKeys transforma "error" → "Error" en todas las respuestas
      const msg = err?.response?.data?.Error
        ?? err?.response?.data?.error
        ?? err?.message
        ?? 'Error al guardar los tiempos';
      console.error('Error guardando tiempos:', err?.response?.data ?? err);
      alert(msg);
    } finally {
